...
| ファイル | 内容 |
| --- | --- |
| [docs/plan/構成案](docs/plan/構成案.md) | **資料を作る側の文書。**各章に何を書いたかの設計記録と、主催者向けの準備、資料のビルド手順 |
...
- **PowerShell 7 を入れておいてください。**5.1 は Windows に標準で入っていますが、この資料は両対応で進めます。導入手順は [03. 実行環境と実行ポリシー](docs/03-実行環境.md) にあります
- **スクリプトが実行できる状態にしておいてください。**実行ポリシーが `Restricted` のままだと第2回以降が進みません（[03.3](docs/03-実行環境.md#033-プロファイルとモジュール)）
