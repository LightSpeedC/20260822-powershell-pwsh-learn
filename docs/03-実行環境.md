...
> 
> この資料を書いた PC は MSI で 7.6.3 を入れていますが、`winget list --id Microsoft.PowerShell` は**「一致するインストール済みのパッケージが見つかりませんでした」** と返します。一方、アンインストール情報（レジストリ）には `PowerShell 7-x64 / 7.6.3.0` として登録されています。
> 
