...
前章: [09. 実務パターン集](09-実務パターン.md) ／ 次章: [11. C# を埋め込む — Add-Type](11-Add-Type.md) ／ [目次に戻る](../README.md)
...
> 
> <strong>「画面の列名 ≠ プロパティ名」</strong>という 05 章の内容が、そのまま試される課題です。
...
> 
> 該当行が**0件のとき `$records` は `$null` になり**、`.Count` が使えません（06.3）。`@($records).Count` と<strong>`@()` で包む</strong>と、0件でも1件でも安全に数えられます。
> 
