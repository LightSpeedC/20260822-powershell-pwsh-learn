...
> **PowerShell が初めての人へ**
> 
> 「オブジェクト」という言葉が難しければ、<strong>「名前の付いた項目をまとめて持ち歩ける箱」</strong>と考えてください。JavaScript のオブジェクト、Java のインスタンス、VBA のオブジェクト変数と同じものです。
> 
...
> **PowerShell が初めての人へ**
> 
> `$_` は<strong>「いま処理している1件」</strong>を表します。JavaScript の `arr.filter(x => ...)` の `x` にあたるものが、名前を書かずに `$_` として用意されている、と考えてください。
> 
...
> 
> <strong>どちらに解決されるかを予測するのは現実的ではありません。</strong>対話操作で使うのは構いませんが、スクリプトには必ずフルネームを書いてください。
