...
[目次に戻る](../README.md) ／ 関連: [A1. 逆引き](A1-逆引き.md) ・ [09. 実務パターン集](09-実務パターン.md)
...
> [!NOTE]
> ****［5.1］** `[CmdletBinding()]` を付けると、`param()` の既定値の中で `$PSScriptRoot` が空文字列になります。**
> 
> そのため `[string]$OutFile = (Join-Path $PSScriptRoot 'result.csv')` と書くと、**引数を省略した瞬間に起動時点で落ちます**。
> `Join-Path : Cannot bind argument to parameter 'Path' because it is an empty string.
> At C:\work\skeleton.ps1:6 char:31
> + [string]$OutFile = (Join-Path $PSScriptRoot 'result.csv')
> + ~~~~~~~~~~~~~`
> `$PSCommandPath` に替えても同じく空です。**［pwsh 7］** では両方とも正しく入るため、**7 で書いて 5.1 で配ると初めて発覚します**。
> 
> `[CmdletBinding()]` を外せば 5.1 でも既定値の中で使えますが、`-Verbose` が効かなくなるので本末転倒です。**既定値は `param()` の外で入れてください。**

> [!NOTE]
> **`$ErrorActionPreference = 'Stop'` は外部コマンドには効きません。**
> 
> これが効くのは Cmdlet の非終了エラーだけです。`robocopy` や `git` のような **exe が異常終了しても例外にはならず**、`catch` を素通りします。外部コマンドの扱いは [A2.5](#a25-外部コマンド呼び出し) を見てください。
...
> 
> `-Path` は値をワイルドカードとして解釈します。`report[2026].txt` のように `[ ]` を含むファイル名は<strong>`-Path` では絶対に見つかりません</strong>。
> 
...
> 
> 登録する前に、`-WorkingDirectory` を明示し、出力を [A2.2](#a22-ログ出力) のログに寄せてください。
